-- Paste this whole file into Supabase SQL Editor and run it from the first line.
-- It creates missing notification tables before any policies/functions refer to them.
...
begin
  ...
  values ('job_created', 'New job request', new.title, new.created_by, new.created_by, 'jobs', new.id);
  return new;
end;
$$;
...
declare
  ...
  job_title text;
begin
  select created_by, title into owner_id, job_title
  from public.jobs
  where id = new.job_id;
...
declare
  recipient_id uuid;
begin
  select case when t.user_id = new.sender_id then t.artisan_id else t.user_id end
  into recipient_id
